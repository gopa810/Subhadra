...
NSMutableDictionary * g_mimeMap;
NSMutableDictionary * g_normalizeMimeMap;
...
+(void)initialize
{
    g_mimeMap = [[NSMutableDictionary alloc] init];
    g_normalizeMimeMap = [[NSMutableDictionary alloc] init];
    
    [g_mimeMap setObject:@"image/png" forKey:@"png"];
    [g_mimeMap setObject:@"image/tiff" forKey:@"tif"];
    [g_mimeMap setObject:@"image/tiff" forKey:@"tiff"];
    [g_mimeMap setObject:@"image/bmp" forKey:@"bmp"];
    [g_mimeMap setObject:@"image/jpg" forKey:@"jpg"];
    [g_mimeMap setObject:@"image/jpg" forKey:@"jpeg"];
    [g_mimeMap setObject:@"image/gif" forKey:@"gif"];
    
    [g_normalizeMimeMap setObject:@"image/png" forKey:@"image/png"];
    [g_normalizeMimeMap setObject:@"audio/mp3" forKey:@"mp3file"];
}

+(NSString *)normalizeFileType:(NSString *)fileType
{
    return [g_normalizeMimeMap objectForKey:fileType];
}

+(NSString *)getMimeType:(NSString *)fileExt
{
    return [g_mimeMap objectForKey:fileExt];
}

+(BOOL)isImageFileExtension:(NSString *)ext
{
    return [FlatFileUtils isImageMimeType:[FlatFileUtils getMimeType:ext]];
}
...
-(id)init
{
    ...
    if (self)
    {
        self.mutableBuffer = [[NSMutableString alloc] init];
        self.extractedTag = [[NSMutableString alloc] init];
    }
    return self;
}
...
{
    [self.mutableBuffer appendFormat:@"%c", c];
}

-(void)appendString:(NSString *)str
{
    [self.mutableBuffer appendString:str];
}

-(void)clear
{
    [self.mutableBuffer setString:@""];
    [_extractedTag setString:@""];
}

-(NSString *)buffer
{
    return [NSString stringWithString:self.mutableBuffer];
}
...
-(NSArray *)createArray
{
	...
	for(int idx = 0; idx < [self.mutableBuffer length]; idx++)
	{
		unichar rd = [_mutableBuffer characterAtIndex:idx];
...
{
    NSString * strCopy = [[NSString alloc] initWithString:str];
    [array addObject:strCopy];
    //[strCopy release];
}
...
-(NSString *)tag
{
    if ([_extractedTag isEqual:@""])
    {
        ...
        int startIdx = 0;
        if ([self.mutableBuffer characterAtIndex:0] == '<')
            startIdx++;
        for (int i = startIdx; i < [self.mutableBuffer length]; i++)
        {
            if ([tagSet characterIsMember:[_mutableBuffer characterAtIndex:i]])
                [_extractedTag appendFormat:@"%C", [_mutableBuffer characterAtIndex:i]];
            else
                break;
        }
    }
...
-(id)init
{
    ...
    if (self) {
        self.buffer = [[NSMutableString alloc] init];
        self.ethStack =[[NSMutableArray alloc] init];
        self.ethDict = [[NSMutableDictionary alloc] init];
...
-(void)reset
{
    self.hcParaStarted = NO;
    self.hcSpanStarted = NO;
    self.hcSub = NO;
    self.hcSup = NO;
    self.hcSupChanged = NO;
    self.hcSubChanged = NO;
    self.hcPwCounter = 0;
    self.hcNtCounter = 0;
    self.hcTableRows = 0;
...
-(void)processChar:(unichar)chr forHtmlTarget:(HtmlString *)target paragraphStyle:(HtmlStyle *)paraStyle characterStyle:(HtmlStyleTracker *)charStyle
{
    ...
    if (self.hcSubChanged && !self.hcSub) {
        [target appendString:@"</sub>"];
    }
    if (self.hcSupChanged && !self.hcSup) {
     ...
     pwLinkStyles:(NSMutableArray *)pwLinkStyle
{
    ...
    if ([str isEqual:@"PW"]) {
        self.hcPwCounter++;
        [pwLevel addObject:[NSNumber numberWithInt:self.hcPwCounter]];
        [pwParaStart addObject:[NSNumber numberWithBool:self.hcParaStarted]];
        self.hcParaStarted = NO;
        [target setAcceptText:(self.hcPwCounter == self.catchPwCounter)];
        ...
        if ([pwParaStart count] > 0) {
            self.hcParaStarted = [(NSNumber *)[pwParaStart lastObject] boolValue];
        ...
        [target setAcceptText:(restCount == self.catchPwCounter)];
        [self checkParagraphStart:target paragraphStyle:paraStyle];

        
        if ([recordDict objectForKey:@"NamedPopup"]) {
            [target appendString:[NSString stringWithFormat:@"<a class=\"LK_%@\" href=\"vbase://inlinepopup/DP/%@/%d\">", classFormat, [FlatFileUtils encodeLinkSafeString:[recordDict valueForKey:@"NamedPopup"]], self.hcPwCounter]];
            self.linkStarted = YES;
        }
        else {
            [target appendString:[NSString stringWithFormat:@"<a class=\"LK_%@\" href=\"vbase://inlinepopup/RD/%@/%d\">", classFormat, [recordDict valueForKey:@"record"], self.hcPwCounter]];
    ...
    {
        self.hcNtCounter++;
        [target setAcceptText:(self.hcNtCounter == self.catchNtCounter)];
    }
    else if ([str isEqual:@"/NT"])
    {
        self.hcNtCounter--;
    ...
    if ([str isEqual:@"BC"]) {
        ...
        self.hcSup = YES;
        self.hcSupChanged = YES;
    }
    else if ([str isEqual:@"SB"]) {
        self.hcSub = YES;
        self.hcSubChanged = YES;
    }
...
-(HtmlString *)htmlStringWithStyles:(HtmlStylesCollection *)styles
                          forRecord:(NSDictionary *)recDict
{
    HtmlString * target = [[HtmlString alloc] init];
    return [self htmlStringWithStyles:styles forRecord:recDict htmlString:target];
}

-(HtmlString *)htmlStringWithStyles:(HtmlStylesCollection *)styles
                          forRecord:(NSDictionary *)recDict
                         htmlString:(HtmlString *)target
{
    @try {
        int charCounter = 0;
        VBRecordNotes * recNotes = nil;
        NSNumber * recNumberId = [recDict objectForKey:@"record"];
        int globalRecordId = -1;
        if (recNumberId != nil)
            globalRecordId = [recNumberId intValue];
        if (globalRecordId >= 0)
            recNotes = [self.validator recordNotesForRecord:(0xffffff & globalRecordId)];
        int recHighlightersIndex = 0;
        VBHighlighterAnchor * highAnch = [recNotes anchorAtIndex:0];
        //NSLog(@"==> record_id (%d) notes (%p)", globalRecordId, recNotes);


        NSString * text = _buffer;
        int status = 0;
        int start = 0;
        int end = 0;
        int tagIdentified = 0;
        self.dataObjectName = nil;
        FlatFileTagString * tag = [[FlatFileTagString alloc] init];
        HtmlStyle * paraStyles = [[HtmlStyle alloc] init];
        self.paraStyleRead = paraStyles;
        HtmlStyleTracker * charStyles = [[HtmlStyleTracker alloc] init];
        NSMutableArray * pwLevel = [[NSMutableArray alloc] init];
        NSMutableArray * pwParaStart = [[NSMutableArray alloc] init];
        NSMutableArray * pwLinkStyle = [[NSMutableArray alloc] init];
        
        [target clear];
        
        
        if (self.catchPwCounter == 0 && self.catchNtCounter == 0)
            [target setAcceptText:YES];
        
        if ([recNotes hasText] && globalRecordId >= 0)
        {
            [target appendString:[NSString stringWithFormat:@"<a href=\"vbase://note/%d\"><img src=\"vbase://stylist_images/note_icon\" style='float:left;margin:8pt' height=40 width=40 border=0></a>", globalRecordId]];
        }
        
        if ([[recDict valueForKey:@"levelName"] length] > 0) {
            [paraStyles setStyleName:[recDict valueForKey:@"levelName"]];
        }
        ...
        for(int i = 0; i < [text length]; i++)
        {
            ...
            if (tagIdentified == 1) {
                ...
                [tag clear];
                @try {
                    [tag appendString:[text substringWithRange:NSMakeRange(start, end - start + 1)]];
                }
                @catch (NSException *exception) {
                    NSLog(@"Invalid range: %d, %d in string %@", start, end - start + 1, text);
                }
                @finally {
                }
                [self processTag:tag forHtmlTarget:target
                      withStyles:styles paragraphStyle:paraStyles characterStyle:charStyles
                          record:recDict pwLevels:pwLevel pwParaStarts:pwParaStart
                    pwLinkStyles:pwLinkStyle];
            } else if (status == 0) {
                while (highAnch && highAnch.startChar == charCounter)
                {
                    [charStyles setValue:highlighterColors[highAnch.highlighterId]
                                  forKey:@"background"];
                    recHighlightersIndex++;
                    highAnch = [recNotes anchorAtIndex:recHighlightersIndex];
                }
                
                [self processChar:[text characterAtIndex:i] forHtmlTarget:target paragraphStyle:paraStyles characterStyle:charStyles];
                charCounter++;
            }
        }
        
        [self finishHtmlFormating:target paragraphStyle:paraStyles
                   characterStyle:charStyles];
        [self finishEtlStarted:target];
        ...
        NSInteger firstPara = [target indexfOfFirstOccurenceOfTag:@"p"];
        if (firstPara != NSNotFound)
        {
            firstPara += 2;
            NSNumber * recordNumber = [recDict objectForKey:@"record"];
            if (recordNumber)
            {
                NSString * strId = [NSString stringWithFormat:@" id=\"rec%@\" ", recordNumber];
                [target insertString:strId atIndex:firstPara];
            }
        }
    }
    @catch (NSException *exception) {
        [target clear];
        [target appendString:@"<p>Fail to load record "];
        [target appendString:[[recDict objectForKey:@"record"] description]];
    }
    @finally {
    }
...
{
    _styleName = aStyleName;
    _styleNameChanged = YES;
}

-(NSString *)valueForKey:(NSString *)str
{
    return [self.format valueForKey:str];
}

-(void)setValue:(NSString *)strValue forKey:(NSString *)strKey
{
    [self.format setValue:strValue forKey:strKey];
}

-(void)clearFormat
{
    [self.format removeAllObjects];
}
...
-(NSString *)styleCssText
{
    ...
    NSEnumerator * enm = [self.format keyEnumerator];
    while((key = [enm nextObject]) != nil)
    {
        ...
        NSString * val = [self.format valueForKey:key];
...
-(id)init
{
    ...
    if (self)
    {
        self.formatOld = [[NSMutableDictionary alloc] init];
        self.formatChanges = [[NSMutableSet alloc]  init];
    }
    return self;
}
...
-(void)setValue:(NSString *)strValue forKey:(NSString *)strKey
{
    ...
    [self.formatChanges addObject:strKey];
}

-(void)clearChanges
{
    self.styleNameChanged = NO;
    [self.formatChanges removeAllObjects];
    [self.formatOld removeAllObjects];
    [self.formatOld setDictionary:self.format];
}
...
-(BOOL)hasChanges
{
    return [self styleNameChanged] || ([self.formatChanges count] > 0);
}
...
-(void)parse
{
    ...
    [self.delegate pushEndfromIndexer:self];
